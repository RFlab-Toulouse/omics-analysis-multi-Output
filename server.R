...
shinyServer(function(input, output,session) {
  ...
  output$download_tsne <- downloadHandler(
    filename = function() { paste('tsne.',  input$paramdownplot, sep='') },
    content = function(file) {
  ...
  output$download_umap <- downloadHandler(
    filename = function() { paste('umap.', input$paramdownplot , sep='') },
    content = function(file) {
      data <- adv_viz_data(); req(data)
  ...
  output$clustered_heatmap <- renderPlot({
    ...
    # }, error =  function(){
    #   errorplot(text = paste("Error generating correlation netwoks :  ", e$message))
    # })
  ...
  MODEL<-reactive({
    ...
    if(input$model == "xgboost"){
      ...
      # alpha/lambda toujours lus (utiles en auto ET en manuel)
      alpha_xgb_model  <- if(!is.null(input$alphaxgb))  input$alphaxgb  else 0
      lambda_xgb_model <- if(!is.null(input$lambdaxgb)) input$lambdaxgb else 1
    ...
    modelparameters<<-list("modeltype"=input$model,"invers"=F,"thresholdmodel"=input$thresholdmodel,
                           ...
                           "alpha_xgb"=alpha_xgb_model, "lambda_xgb"=lambda_xgb_model,
  ...
  output$xgbsubsample <- renderText({
    if(input$model == "xgboost" && !is.null(MODEL()$MODEL)){
      val <- MODEL()$MODEL$optimal_subsample
      if(!is.null(val)) format(val, digits = 3) else "N/A"
    } else {
      "N/A"
    }
  })
  ...
  output$pca_plot_2d_stats <- renderPlotly({
    ...
    #   errorplot(text = "Error generating PCA 2D plot: not enough variables or data issue ")
    # }
  })
  ...
  output$plot_lc_auc <- renderPlot({
    ...
    req(lc_data)
    tryCatch({
      plot_learning_curve_multiclass(lc_data, metric = "auc", title = "Learning Curve - AUC (macro)")
    }, error =  function(e){
  ...
  output$download_lc_auc <- downloadHandler(
    ...
    content = function(file) {
      lc_data <- LEARNING_CURVE_DATA()
      req(lc_data)
      p <- plot_learning_curve_multiclass(lc_data, metric = "auc", title = "Learning Curve  AUC (macro)")
...
COMPARISON = eventReactive(input$run_comparison, {
  ...
  delong_results <- reactive({
    req(COMPARISON())
    delong_compare_models(COMPARISON())
  })
  
  output$delong_test_table <- renderDataTable({
    delong_results()$global
  }, options = list(pageLength = 10, dom = 't'))
  
  output$delong_corrected_table <- renderDataTable({
    delong_results()$corrected
  }, options = list(pageLength = 10, dom = 't'))
  
  output$delong_perclass_table <- renderDataTable({
    delong_results()$per_class
  }, options = list(pageLength = 15, dom = 'tp'))
  
  output$download_delong_table <- downloadHandler(
    filename = function() { paste('delong_global.', input$paramdowntable, sep='') },
    content = function(file) { downloaddataset(delong_results()$global, file) })
  
  output$download_delong_corrected <- downloadHandler(
    filename = function() { paste('delong_fdr_corrected.', input$paramdowntable, sep='') },
    content = function(file) { downloaddataset(delong_results()$corrected, file) })
  
  output$download_delong_perclass <- downloadHandler(
    filename = function() { paste('delong_per_class.', input$paramdowntable, sep='') },
    content = function(file) { downloaddataset(delong_results()$per_class, file) })
